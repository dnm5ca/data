...
imdb %>%
  filter(Metascore < 20  & (Revenue >= 100 | Revenue < 600)) %>% 
  select(Title, Metascore, Revenue)
...
imdb %>%
  filter(Runtime > 150 & Runtime < 180) %>%
...
boxoffice_wide %>% 
  ...
  gather(key = Year, value = Total_Gross, Total_Gross2016:Total_Gross2006)  

#make that change stick
Total_Gross <- boxoffice_wide %>% 
  select(-starts_with("Average"), -ID) %>% 
  gather(key = Year, value = Total_Gross, Total_Gross2016:Total_Gross2006)

#Let's use mutate to get rid of the Total_Gross portion of year. 
Total_Gross <- Total_Gross %>% 
  mutate(Year = as.numeric(str_replace(Year, "Total_Gross", "")))

#if we wanted to put it back to the way it was, we can reverse it using spread, which takes things from long to wide
spread(Total_Gross, Year, Total_Gross)
...
```{r, results="hide"}
View(imdb)
imdb <- imdb %>% separate(Actors, into = c("Primary_Actor", "Secondary_Actor"), sep = ",")
...
```{r}
#Let's do the same for Average
Ave <- boxoffice_wide %>% 
  select(-starts_with("Total_Gross"), -ID) %>% 
  gather(key = Year, value = Average, Average2016:Average2006)  

#Let's use mutate to get rid of the Total_Gross portion of year. 
Ave <- Ave %>% 
  mutate(Year = as.numeric(str_replace(Year, "Average", "")))
```
...
summary(joined)
names(joined)
joined <- joined %>% mutate(imdb_rating = Rating * 10) %>% 
  mutate(rating_ave = (imdb_rating + Audience_Rating + RT_Rating)/3,
         Revenue_Rank = rank(Revenue, na.last = "keep"),
         dir_actor = paste(Director, Primary_Actor)) 

##Case_When (Useful Function)
joined <- joined %>%
  mutate(Rating_Level = case_when(
    Rating < 4 ~ "Low", 
    Rating >= 4 & Rating < 7.5 ~ "Medium", 
    Rating >= 7.5 ~ "High")
  )
```
...
joined <- joined %>%
  select(Title, imdb_rating, RT_Rating, Revenue, everything())

```

Our last tidying step will be to remove cases where Revenue was missing.
```{r, results="hide"}
summary(joined$Revenue)

clean <- joined %>%
  filter(!is.na(Revenue))

summary(clean$Revenue)
...
clean %>%
  filter(Primary_Actor == "Christian Bale")
...
clean %>% 
  filter(str_detect(Primary_Actor, "[Cc]hr")) %>% #Gets any matches with Chr/chr in it.  
  select(Title, Primary_Actor)

#However, we only want to look at the start of the string. We use the ^ to signify that.
clean %>% 
  filter(str_detect(Primary_Actor, "^[Cc]hr")) %>% 
  select(Title, Primary_Actor)
...
clean %>% 
  filter(str_detect(Primary_Actor, "^[Cc]hr") & str_detect(Primary_Actor, "ale$")) %>% 
  select(Title, Primary_Actor)
```
...
clean %>%
  filter(Director == "Ridley Scott" & Metascore > 50)
...
clean %>% 
  summarize(n_prim_actors = n_distinct(Primary_Actor))
```
...
clean %>%
  group_by(Genre1) %>%
  summarize(meanrev=mean(Revenue)) %>% 
  rename('mean_Revenue' = "meanrev")

clean %>%
  filter(Metascore > 80 & (Genre1 == "Action" | Genre1 == "Animation")) %>% 
  group_by(Year) %>% 
  summarize(mean(Revenue))

clean %>% 
  filter(!is.na(RT_Rating) & !is.na(Metascore)) %>% 
  summarize(cor(Metascore, RT_Rating), 
            mean(Metascore), 
            mean(RT_Rating), 
            cor(Metascore, Revenue))

clean$Total_Gross

clean %>% 
  mutate(perc_of_Gross = (Revenue/Total_Gross) * 100) %>% 
  select(Year, Title, Revenue, perc_of_Gross) %>% 
  arrange(desc(perc_of_Gross))


plot(clean$Metascore, clean$RT_Rating)
plot(clean$Metascore, clean$Revenue)
```
...
clean %>% 
  filter(Primary_Actor == "Leonardo DiCaprio" & Metascore > 65) %>%
  select(Title, Metascore, Revenue) %>% 
...
clean %>% 
  filter(Metascore < 50 & Genre1 == "Action") %>% 
  arrange(desc(Revenue)) %>% 
  head(4) %>% 
  select(Title, Metascore, Revenue) 
...
clean %>% 
  filter(Metascore < 50 & Revenue > 30) %>% 
  filter(!is.na(imdb_rating), !is.na(Metascore)) %>% 
  group_by(Year) %>% 
  summarize(meanmeta = mean(Metascore), 
            meanimdb = mean(imdb_rating), 
            corrating = cor(Metascore, imdb_rating))
...
?num_range
clean %>% 
  filter(Metascore >=  70 & str_detect(Genre1, "^[A]")) %>%
  select(Title, num_range("Genre", 1:3))
...
str_detect("This is love isn't it", "^(.*)love,|^(.*)love\\.$|^(.*)love$")

love <- str_detect(clean$Description, "^(.*)love,|^(.*)love\\.$")
clean$Description[love]
```